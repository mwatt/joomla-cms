...
class JTableNested extends JTable
{
	...
	protected function triggerPublished($pks, $only_children=false)
	{
		...
		foreach ($pks as $pk)
		{
			...
			// based on p.path_published and own published column,
			// where (p) is parent category is and (c) current category
			//
			// p.path_published <=  c.published AND p.path_published > 0 THEN c.published
			...
			//
			// p.path_published >   c.published AND c.published > 0 THEN p.path_published
			//                2 >   1 THEN  2 (If published in archived then archived)
			//
			// p.path_published >   c.published THEN c.published ELSE p.path_published
			...
			$query->clear()
				->update($table . ' AS c')
				->join('LEFT', $table . ' AS p ON c.parent_id = p.' . $k)
				->set('path_published = ' .
					'CASE WHEN p.path_published <= c.published AND p.path_published > 0 THEN c.published' .
